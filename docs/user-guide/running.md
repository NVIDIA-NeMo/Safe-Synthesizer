...
| `training.nemotron_capabilities` | config | Nemotron training uses the BF16 checkpoint without dynamic quantization or differential privacy |
...
differential privacy. Experimental generation may use the exact official
`nvidia/NVIDIA-Nemotron-3-Nano-4B-FP8` sibling by setting
`generation.pretrained_model`. This FP8 generation path requires compute
...
all dynamic training quantization schemes.
