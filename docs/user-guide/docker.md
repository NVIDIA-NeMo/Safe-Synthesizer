...
| `NVIDIA_API_KEY` | For PII classification | API key for the NIM endpoint. Set when using the CLI (or container) for column classification |
| `NIM_ENDPOINT_URL` | For PII classification | NIM/OpenAI-compatible endpoint URL (default: `https://integrate.api.nvidia.com/v1`). Override for a custom endpoint |
