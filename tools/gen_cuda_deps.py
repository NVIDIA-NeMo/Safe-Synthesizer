...
def load_pyproject_dependency_overrides(path: Path) -> list[str]:
    """Load project-wide uv dependency overrides for the standalone installer."""
    data = tomllib.loads(path.read_text(encoding="utf-8"))
    tool = data.get("tool")
    uv = tool.get("uv") if isinstance(tool, dict) else None
    overrides = uv.get("override-dependencies", []) if isinstance(uv, dict) else []
    if not isinstance(overrides, list) or not all(isinstance(override, str) for override in overrides):
        raise ValueError("pyproject.toml: [tool.uv].override-dependencies must be an array of strings")
    return overrides
...
def build_cuda_installer_fragment(
    config: CudaDepsConfig,
    dependency_overrides: Iterable[str] = (),
) -> CudaInstallerFragment:
    """Build generated shell arrays for project overrides and each installer runtime extra."""
    lines = [
...
def run_generation_command(
    ...
    dependency_overrides = load_pyproject_dependency_overrides(pyproject_path)
    generated_installer = build_cuda_installer_fragment(config, dependency_overrides)
