...
* ``safe-synthesizer run``: full end-to-end pipeline.
* ``safe-synthesizer run train``: process data and train only.
* ``safe-synthesizer run generate``: load a trained adapter and generate only.

``common_run_options`` supplies the shared operational options for those
commands: config and data paths, artifact and run paths, output file, logging,
WandB, dataset registry, and verbosity. Environment-variable handling stays in
``CLISettings`` rather than Click ``envvar=`` declarations, so precedence is
centralized in one settings model.

Safe Synthesizer parameter overrides are generated by
``pydantic_options(SafeSynthesizerParameters, field_separator="__")``. Keep
that separator aligned with ``parse_overrides`` so nested options such as
``--data__holdout`` become nested config dictionaries.
"""
