...
class EntityExtractorGliner(EntityExtractor):
    ...
    def _detect_entities_chunked(
        ...
        if entity_labels is None or model is None:
            return []
        predict_entities = getattr(model, "predict_entities")
        ...
        while start < len(text):
            ...
            if temp_entities is None:
                n_cache_miss += 1
                temp_entities = predict_entities(
                    chunk,
                    entity_labels,
    ...
    def batch_update_cache(self, texts: list[str], entities: Optional[set[str]] = None):
        ...
        if entities is None or model is None:
            return
        batch_predict_entities = getattr(model, "batch_predict_entities")
        ...
        for batch_n, batch in enumerate(
            ...
            entities_lists = batch_predict_entities(
                batch,
                entities,
                threshold=self._ner_threshold,
                flat_ner=False,
