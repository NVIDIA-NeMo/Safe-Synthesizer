...
class PlanIssue:
    ...
    Args:
        code: Stable issue identifier for logs and troubleshooting docs.
        message: Human-readable explanation of the problem.
        severity: ``error`` blocks the plan; ``warning`` is advisory only.
    """
...
def _iter_column_spec_issues(df: pd.DataFrame, plan: PiiReplacementPlan, df_cols: set[str]) -> Iterator[PlanIssue]:
    ...
    for location, spec in _column_specs(plan):
        ...
        if not spec.patterns:
            continue
        entity_spec = entities.spec(spec.entity_type.value)
        if entity_spec is not None and entity_spec.valid_form:
            yield PlanIssue(
                "pii_plan_pattern_invalid",
                f"column {col!r} sets patterns, but a {spec.entity_type.value} address is generated by a "
...
def _iter_section_placement_advisories(
    ...
    *,
    ...
    """Warn when a column's YAML section does not match how the engine will treat it.

    Entity type (and persona backend) still decide the replacement path. Listing
    a persona-sourced column only under ``standalone_columns_to_replace`` does
    change consistency: it will not share a synthetic person with other columns.
    These advisories surface mismatches so a hand-edited plan can be cleaned up.
    """
    persona_expected = {
        label
    ...
    for col_set in plan.persona_backed_columns:
        ...
        for spec in col_set.columns_to_replace:
            ...
            elif entities.effective_apply_path(label, persona_backend) == "standalone_map":
                yield PlanIssue(
                    "pii_plan_entity_driven_under_persona",
                    f"{loc} lists {spec.column_name!r} as {label}; that entity is replaced from its "
                    "own value, not from the persona, so listing it under persona_backed_columns "
                    "does not give persona consistency.",
                    severity="warning",
                )
