...
"""Character templates for identifiers, phones and cards: infer, match, and generate."""
...
from .evidence import dominant_format, pattern_evidence_values, ranked_formats

logger = get_logger(__name__)


def value_patterns(values: pd.Series, cfg: Config) -> list[str]:
    """Return zero or one character template for plan emission.
    ...
    a shape group, ``infer_value_pattern`` still widens classes. The plan names at
    most one template: the dominant shape must cover ≥
    ``cfg.dominant_pattern_min_coverage`` of the evidence sample.
    ...
    Returns:
        ``[template]`` when a dominant format exists, else ``[]``.

    Example:
        ``["pmc-6123", "pmc-8123", "pmc-6124"]`` -> ``["pmc-[68]###"]``.
    """
    sample = pattern_evidence_values(values)
    pattern = _dominant_value_pattern(sample, cfg, value_shape_template)
    if pattern is None:
        pattern = _dominant_value_pattern(sample, cfg, value_structure_template)
    return [pattern] if pattern else []


def value_pattern(values: pd.Series, cfg: Config) -> str | None:
    """Dominant character template, or ``None`` when coverage is below threshold."""
    patterns = value_patterns(values, cfg)
    return patterns[0] if patterns else None


def ranked_value_patterns(values: pd.Series, cfg: Config) -> list[str]:
    """Multi-format ranking for inference internals (not plan emission).

    Uses ``PATTERN_MIN_SHARE`` / ``PATTERN_MAX_PATTERNS``. Prefer ``value_patterns``
    when attaching a plan ``pattern``.
    """
    sample = pattern_evidence_values(values)
...
def _dominant_value_pattern(sample: list[str], cfg: Config, shape_fn) -> str | None:
    ...
    # Denominator is the evidence sample size (not just recognized shapes).
    top_shape = dominant_format(counts, len(sample), min_coverage=cfg.dominant_pattern_min_coverage)
    if top_shape is None:
        return None
    return infer_value_pattern(by_shape[top_shape], cfg)


def _value_patterns_for_shape_fn(sample: list[str], cfg: Config, shape_fn) -> list[str]:
    """Internal multi-shape ranking (email handle enumeration); not for plan emission."""
    by_shape: dict[str, list[str]] = {}
    counts: Counter = Counter()
    for value in sample:
        shape = shape_fn(value)
...
def _entity_allows_template(label: str) -> bool:
    try:
        entity_type = EntityType(label)
    except ValueError:
        return False
    return ENTITY_BY_TYPE[entity_type].pattern_kind is PatternKind.template


def attach_value_patterns(df: pd.DataFrame, standalone: list[DetectedStandalone], cfg: Config) -> None:
    """Attach at most one character template per standalone column, in place.

    A phone column of ``+1-415-555-####`` values gets
    ``ent.pattern = "+1-###-###-####"`` when that format dominates. Temporal /
    identify-only entities are left alone (strftime). Types with
    ``pattern_kind=none`` never get a pattern.

    Args:
        df: Source dataframe.
        standalone: Detected standalone columns to mutate with ``pattern``.
        cfg: Pattern inference configuration.
    """
    for ent in standalone:
        ...
        if ent.entity == "date_of_birth" or is_identify_only(ent.entity):
            continue
        if not _entity_allows_template(ent.entity):
            ent.pattern = None
            continue
        if ent.column not in df.columns:
            continue
        ent.pattern = value_pattern(df[ent.column].dropna(), cfg)
        logger.runtime.info(f"[PII Replacement] Standalone column {ent.column!r} writes pattern={ent.pattern!r}")
