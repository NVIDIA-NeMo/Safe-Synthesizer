...
def build_non_person_maps(
    ...
    """Build scoped non-person maps: col -> scope_key -> {original: synthetic}."""
    gk = runtime_plan.get("group_key")
    scoped: dict[str, dict[Any, dict[str, str]]] = {}

    for ent in runtime_plan.get("non_person", []):
        col, entity = ent["column"], ent["entity"]
        if col not in original_df.columns:
            continue
        # Temporal columns are identified only to keep them out of free-text; they are
        # never value-replaced. Birth dates are handled as a person field.
        ...
        def _map_column(frame: pd.DataFrame) -> dict[str, str]:
            fake = core._seeded_faker(cfg.random_seed + core._stable_hash(col), cfg.locale)
            rng = fake.random
            originals = [str(v) for v in frame[col].dropna().unique()]
            used = set(originals)
            ...
            for sv in originals:
                ...
                if pattern:
                    for _ in range(50):
                        cand = core.generate_from_pattern(pattern, rng)
                        if cand and cand != sv and cand not in used:
                            new = cand
                            break
                if new is None:
                    for _ in range(50):
                        cand = core._fake_value(entity, sv, fake)
                        if cand and cand != sv and cand not in used:
                            new = cand
                            break
                if new is None:
                    new = core._fake_value(entity, sv, fake)
                if new and new != sv:
                    mapping[sv] = new
                    used.add(new)
            return mapping
        ...
        if gk and gk in original_df.columns:
            for gval, gdf in original_df.groupby(gk, dropna=True):
                scoped[col][gval] = _map_column(gdf)
        else:
            for idx, row in original_df.iterrows():
                frame = original_df.loc[[idx]]
                scoped[col][idx] = _map_column(frame)
...
def apply_replacements(
    ...
    original_df: pd.DataFrame,
    instances: list[dict],
    non_person_maps: dict[str, dict[Any, dict[str, str]]],
    runtime_plan: dict[str, Any],
    ...
    for col, scope_maps in non_person_maps.items():
        if col not in replaced_df.columns:
            continue
        if group_key and group_key in original_df.columns:
            for gval, mapping in scope_maps.items():
                if not mapping:
                    continue
            ...
            mapping = scope_maps[""]
            if mapping:
                mapped = original_df[col].map(lambda v: mapping.get(core._sval(v)))
                replaced_df[col] = mapped.where(mapped.notna(), replaced_df[col])
        else:
            for idx, mapping in scope_maps.items():
                if not mapping:
                    continue
    ...
    def _row_pairs(idx) -> list[tuple[str, str]]:
        ...
        for col, scope_maps in non_person_maps.items():
            ...
            if ov is None:
                continue
            if group_key and group_key in original_df.columns:
                gval = original_df.at[idx, group_key]
                mapping = scope_maps.get(gval, {})
            else:
                mapping = scope_maps.get(idx, {}) or scope_maps.get("", {})
            if ov in mapping:
    ...
    if ft_cols and (group_text_pairs or record_text_by_cols or non_person_maps):
        ...
        for col, scope_maps in non_person_maps.items():
            ...
            for mapping in scope_maps.values():
                for ov in mapping:
