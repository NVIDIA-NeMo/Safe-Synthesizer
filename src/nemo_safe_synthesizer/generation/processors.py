...
from ..data_processing.value_post_processor import ValuePostProcessor
...
class Processor(ABC):
    ...
    def __init__(
        ...
        logger.debug(
            f"Initialized processor with schema={schema}, config={config}, "
            f"serialization_format={serialization_format}, tokenizer_present={tokenizer is not None}"
        )
...
class TabularDataProcessor(Processor):
    ...
    def __init__(
        ...
        self._value_post_processor: ValuePostProcessor | None = None
        self._column_order: list[str] | None = None
        if serialization_format == "positional":
            self._value_post_processor = ValuePostProcessor(schema)
            self._column_order = self._value_post_processor.columns

    def _process_text_generation(self, text: str) -> ParsedResponse:
        """Extract and validate records from the generated text.

        Dispatches on ``self.serialization_format``:

        * ``"json"`` (default): regex-extract JSON rows and validate with
          ``jsonschema`` via :func:`extract_and_validate_records`.
        * ``"positional"``: split on ``<|sep|>``/``<|eor|>`` sentinels and
          impose the schema per-column via
          :func:`extract_and_validate_positional_records`.
        """
        if self.serialization_format == "positional":
            assert self._value_post_processor is not None and self._column_order is not None
            return extract_and_validate_positional_records(
                text,
                processor=self._value_post_processor,
...
def create_processor(
    ...
    if config.time_series.is_timeseries:
        if serialization_format == "positional":
            raise ValueError(
                "serialization_format='positional' is not yet supported with time-series data; "
                "use 'json' or disable time_series."
            )
        ...
        if serialization_format == "positional":
            raise ValueError(
                "serialization_format='positional' is not yet supported with "
                "group_training_examples_by; use 'json' or disable grouping."
            )
