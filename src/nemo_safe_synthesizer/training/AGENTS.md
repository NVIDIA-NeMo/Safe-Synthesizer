...
- `trust_remote_code_for_model()` — from `llm.utils`; returns `True` for NVIDIA Hub IDs and Hugging Face hub cache paths with a `models--nvidia--*` segment. Called directly in `HuggingFaceBackend` for all `from_pretrained` calls.
