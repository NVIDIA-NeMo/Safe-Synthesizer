...
def test_non_llm_mode_skips_free_text_scan_without_structured_columns(caplog):
    ...
    # No same-person columns and no replaceable standalone columns: only a generic
    ...
    assert column_spec(plan, "notes") is None
...
def test_free_text_planned_with_standalone_only_structured_columns():
    ...
    assert any(s.entity_type == EntityType.unique_identifier for s in plan.columns_to_replace)
    notes = column_spec(plan, "notes")
    assert notes is not None and notes.entity_type == EntityType.free_text
