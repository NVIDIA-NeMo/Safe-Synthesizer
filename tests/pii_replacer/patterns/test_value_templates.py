...
"""Character templates: shapes a column of values wears, and the dominant one."""
...
def test_dominant_pattern_names_only_the_majority_shape():
    """Plan emission keeps the top format only when it covers ≥ 85% of the sample."""
    from nemo_safe_synthesizer.pii_replacer.entities import Config
    from nemo_safe_synthesizer.pii_replacer.patterns import value_pattern, value_patterns

    # 80% PMC / 20% ACC — under 85%, so plan pattern is omitted.
    mixed = pd.Series(
        [f"PMC-{(i * 24851) % 1000000:06d}" for i in range(40)] + [f"ACC-{(i * 937) % 10000:04d}" for i in range(10)]
    )
    assert value_pattern(mixed, Config()) is None

    # 90% PMC — dominant shape is named.
    dominant = pd.Series(
        [f"PMC-{(i * 24851) % 1000000:06d}" for i in range(45)] + [f"ACC-{(i * 937) % 10000:04d}" for i in range(5)]
    )
    assert value_pattern(dominant, Config()) == "PMC-######"
    # Internal ranking still lists secondary shapes for inference helpers.
    assert value_patterns(dominant, Config()) == ["PMC-######"]


def test_a_shape_too_few_values_wear_is_left_unnamed():
    from nemo_safe_synthesizer.pii_replacer.entities import Config
    from nemo_safe_synthesizer.pii_replacer.patterns import value_pattern

    values = pd.Series([f"PMC-{(i * 24851) % 1000000:06d}" for i in range(99)] + ["ACC-0001"])
    assert value_pattern(values, Config()) == "PMC-######"
...
def test_unterminated_character_class_is_rejected():
    ...
    with pytest.raises(ValueError, match="unterminated character class"):
        generate_from_pattern("pmc-[68", Random(0))
