...
class TestNSSTrainingAndGenerationEvent:
    def test_defaults(self):
        event = NSSTrainingAndGenerationEvent(task="run", taskStatus=TaskStatusEnum.COMPLETED)
    ...
    def test_all_task_statuses(self):
        for status in TaskStatusEnum:
            event = NSSTrainingAndGenerationEvent(task="run", taskStatus=status)
            assert event.task_status == status

    def test_all_deployment_types(self):
        for dtype in DeploymentTypeEnum:
            event = NSSTrainingAndGenerationEvent(task="run", taskStatus=TaskStatusEnum.COMPLETED, deploymentType=dtype)
    ...
    def test_metrics(self):
        event = NSSTrainingAndGenerationEvent(
            ...
            taskStatus=TaskStatusEnum.COMPLETED,
            jobDurationSec=42.5,
            numRecordsGenerated=1000,
            numTokensGenerated=50000,
            inputRecordsBucket=bucket_records(500),
            inputColumnsBucket=bucket_columns(8),
            syntheticQualityScore=0.87,
            dataPrivacyScore=0.95,
...
class TestBuildPayload:
    def _make_queued(self, task: str = "generate", status: TaskStatusEnum = TaskStatusEnum.COMPLETED) -> QueuedEvent:
        event = NSSTrainingAndGenerationEvent(task=task, taskStatus=status)
...
class TestTelemetryDisabled:
    def test_enqueue_noop_when_disabled(self):
        ...
        event = NSSTrainingAndGenerationEvent(task="generate", taskStatus=TaskStatusEnum.COMPLETED)
